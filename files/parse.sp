...
{
    //new String:szFileToDownload[128];
    char szFileToDownload[32];
    Format(szFileToDownload, sizeof(szFileToDownload), "eItems.json");//Generally, it is items.json or eitems.json
    PrintToServer("%s Downloading eItems data from API", TAG_NCLR);
    HTTPClient httpClient = new HTTPClient("This is the URL address of your API");
...
public void PraseItemsDownloaded(HTTPResponse response, any value)
{
    ...
    if (response.Status != HTTPStatus_OK)
    {
        ...
        PrintToServer("%s Downloading eItems data from API failed!!", TAG_NCLR);
